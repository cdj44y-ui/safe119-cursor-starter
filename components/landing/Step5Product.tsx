        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-14">
          {processSteps.map((ps, i) => (
            <div key={ps.num} className="bg-white border border-sand-200 rounded-lg p-5 text-center relative">
              ...
              <div className="text-xs text-ink-5">{ps.desc}</div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {nineElements.map((el) => (
            <div key={el.code} className="bg-white border border-sand-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              ...
              <div className="text-xs font-bold text-copper mb-0.5">{el.code}.</div>
              <div className="text-sm font-bold text-ink leading-snug mb-1">{el.name}</div>
              <div className="text-xs text-ink-5">{el.article}</div>
            </div>
          ))}
        </div>
      </div>
